...
function DatabaseRoulettePlays:create(uuid, playerId, reward) 
	db.asyncQuery(('INSERT INTO `roulette_plays` (player_id, uuid, reward_id, reward_count) VALUES (%d, %s, %d, %d)'):format(
		playerId,
...
function DatabaseRoulettePlays:update(uuid, status)
	db.asyncQuery(('UPDATE `roulette_plays` SET `status` = %d, `updated_at` = %d WHERE `uuid` = %s'):format(
		status,
		os.time(),
		db.escapeString(uuid)
	))
...
function DatabaseRoulettePlays:updateAllRollingToPending()
	db.asyncQuery('UPDATE `roulette_plays` SET `status` = 1 WHERE `status` = 0')
end
